...
- [ ] T005 Add package details query keys for package, validation, and manifest resources in `src/Elsa.Catalog.AdminUi/src/lib/query/queryClient.tsx`
...
- [ ] T020 [P] [US2] Add UI tests for version list badges, version switching, missing version recovery, and version-plus-section deep links in `src/Elsa.Catalog.AdminUi/src/features/packages/PackageDetailsPage.test.tsx`
- [ ] T021 [P] [US2] Add E2E smoke test for package details route, version route, and section route navigation in `tests/Elsa.Catalog.AdminUi.E2E/package-details.spec.ts`

### Implementation for User Story 2

- [ ] T022 [US2] Implement version selector/list with approval, validation, listed, suspicious, and indexed timestamp display in `src/Elsa.Catalog.AdminUi/src/features/packages/PackageDetailsPage.tsx`
- [ ] T023 [US2] Implement URL synchronization for package-level default version, explicit version routes, and major section routes in `src/Elsa.Catalog.AdminUi/src/features/packages/PackageDetailsPage.tsx`
- [ ] T024 [US2] Add recoverable version-not-found and unavailable section states in `src/Elsa.Catalog.AdminUi/src/features/packages/PackageDetailsPage.tsx`
- [ ] T025 [US2] Update route definitions for `/admin/packages/:packageId/versions/:version` and `/admin/packages/:packageId/versions/:version/:section` in `src/Elsa.Catalog.AdminUi/src/app/routes.tsx`
...
- [ ] T026 [P] [US3] Add admin API tests for normalized validation findings with errors, warnings, missing code/path, no findings, and package/version not found in `tests/Elsa.Catalog.Api.Tests/AdminValidationApiTests.cs`
- [ ] T027 [P] [US3] Add UI tests for validation grouping, valid empty state, suspicious hash evidence, multiple blocker groups, and validation-load failure state in `src/Elsa.Catalog.AdminUi/src/features/packages/PackageDetailsPage.test.tsx`

### Implementation for User Story 3

- [ ] T028 [US3] Update admin validation endpoint to return normalized `findings` with severity, code, message, path, blocking impact, validated timestamp, and validator version in `src/Elsa.Catalog.Api/Admin/Packages/AdminValidationEndpoints.cs`
- [ ] T029 [US3] Add validation findings API response types and normalization for historic result payload shapes in `src/Elsa.Catalog.AdminUi/src/features/packages/packageModels.ts`
- [ ] T030 [US3] Implement validation and visibility sections with grouped blockers, valid state, suspicious hash evidence, and scoped validation failure state in `src/Elsa.Catalog.AdminUi/src/features/packages/PackageDetailsPage.tsx`
- [ ] T031 [US3] Add validation and visibility search/filter helpers for severity, blocking state, code, path, and message in `src/Elsa.Catalog.AdminUi/src/features/packages/packageModels.ts`
...
```text
Task: "T013 [P] [US1] Add admin API tests for package details summary, source summary, canonical casing, latest indexed default data, and not-found behavior in tests/Elsa.Catalog.Api.Tests/AdminPackagesApiTests.cs"
Task: "T014 [P] [US1] Add Package Details UI tests for summary rendering, latest indexed default selection, visibility reasons, canonical casing, and not-found state in src/Elsa.Catalog.AdminUi/src/features/packages/PackageDetailsPage.test.tsx"
```

## Parallel Example: User Story 3

```text
Task: "T026 [P] [US3] Add admin API tests for normalized validation findings with errors, warnings, missing code/path, no findings, and package/version not found in tests/Elsa.Catalog.Api.Tests/AdminValidationApiTests.cs"
Task: "T027 [P] [US3] Add UI tests for validation grouping, valid empty state, suspicious hash evidence, multiple blocker groups, and validation-load failure state in src/Elsa.Catalog.AdminUi/src/features/packages/PackageDetailsPage.test.tsx"
```
