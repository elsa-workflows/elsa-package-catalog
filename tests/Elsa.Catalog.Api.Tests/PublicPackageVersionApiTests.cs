...
public sealed class PublicPackageVersionApiTests
{
    ...
    public async Task Get_version_returns_feature_settings()
    {
        ...
        var version = await app.CreateClient().GetFromJsonAsync<PublicPackageVersionResponse>("/api/packages/Elsa.Email/versions/1.0.0");
    ...
    public async Task Get_hidden_version_returns_not_found()
    {
        ...
        var response = await app.CreateClient().GetAsync("/api/packages/Elsa.Email/versions/1.0.0");
